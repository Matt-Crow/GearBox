...
public class ItemStackTester
{
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new ItemStack(new Item(new ItemType("foo")), -1));
    }

    [Fact]
    public void Constructor_CanAcceptEmptyStack()
    {
        var stackable = new ItemStack(new Item(new ItemType("foo")), 0);
        Assert.Equal(0, stackable.Quantity);

        var nonStackable = new ItemStack(new Item(new ItemType("bar")), 0);
    ...
    {
        var sut = new ItemStack(new Item(new ItemType("foo")));
